...
def roc_auc_ci(
    ...
    *,
    ...
    y_pred = torch.as_tensor(y_pred).float()
    if y_true.shape != y_pred.shape:
        raise ValueError(
            f"y_true and y_pred should have the same shape; "
            f"got {y_true.shape} and {y_pred.shape}"
    ...
    try:
        ...
        bootstrap_aucs = roc_auc(
            y_true_bootstraps.flatten(1), y_pred_bootstraps.flatten(1)
        )
        ...
        return CIResult(estimate, lower, upper)
    except ValueError:
        return CIResult(float("nan"), float("nan"), float("nan"))


def acc_ci(y_true, y_pred, *, level=0.95, method="wilson"):
    """
    Compute a confidence interval for an accuracy (proportion exact match correct).
    """
    ...
    if (
        not ((y_true == 1) | (y_true == 0)).all()
        and ((y_pred == 1) | (y_pred == 0)).all()
    ):
        raise ValueError("y_true and y_pred should contain only 0s and 1s")
    y_true, y_pred = y_true.long(), y_pred.long()

    # Calculate the number of correct predictions
    correct = (y_true == y_pred).sum()

    # Compute the confidence interval using the statsmodels library
    ci = proportion_confint(correct, len(y_true), alpha=1 - level, method=method)
    return CIResult(
        float(correct / len(y_true)),
        assert_type(float, ci[0]),
        assert_type(float, ci[1]),
    )
